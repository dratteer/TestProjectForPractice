...
public class ProductController {
    ...
    @PostMapping()
    public ResponseEntity<IdDto> create(@RequestBody ProductCreateUpdateDto dto) {
        IdDto saved = productService.create(dto);
        return ResponseEntity.status(HttpStatus.CREATED).body(saved);
    }

    @PutMapping("/{id}")
    public void update(@PathVariable Long id, @RequestBody ProductCreateUpdateDto productDtoResponse) {
